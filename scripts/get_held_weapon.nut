...
/*
	About:
		GetHeldWeaponName() returns currently held weapon of player as a STRING
		GetHeldWeaponHandle() returns currently held weapon of player as a HANDLE

	Manual:
		RunscriptCode GetHeldWeaponName(player)
		Where 'player' is a handle of targeted player. Such as Entities.FindByName(null, "Bob")
*/

function GetHeldWeaponName(handle = null){ // Returns the currently held weapon as a string
...
function GetHeldWeaponHandle(handle = null){ // Returns the currently held weapon as a handle
	if (handle == null || typeof handle != "instance")return null; // Check for correct inputs
	local weapon = null;
